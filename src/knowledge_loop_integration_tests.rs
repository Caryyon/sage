...
mod tests {
    ...
    fn test_simple_query_routes_correctly() {
        ...
        assert_eq!(kl.history().len(), 2, "Should have user + assistant messages");
        assert!(kl.active_cells() > 0, "Knowledge should be encoded in NCA grid");
        
        // Verify LLM was called with the query
        let calls = engine.get_calls();
        assert_eq!(calls.len(), 1, "Should call LLM once");
        assert!(calls[0].contains("What is SAGE"), "LLM should receive the query");
    }
    ...
    fn test_complex_query_uses_llm() {
        ...
        assert_eq!(result, "LLM response", "Complex query should use LLM backend");
        
        // Verify the query was passed to LLM
        let calls = engine.get_calls();
        assert!(calls.iter().any(|c| c.contains("converge")), "Query should reach LLM");
    ...
    fn test_empty_predictor_falls_back_to_llm() {
        ...
        assert!(kl.active_cells() > 0, "Should encode knowledge with LLM fallback");
    ...
    fn test_knowledge_retrieval_returns_context() {
        ...
        if let Some(ctx) = context {
            assert!(ctx.contains("Recalled Knowledge") || ctx.contains("speed"),
    ...
    fn test_clear_history_preserves_brain() {
        ...
        assert_eq!(kl.active_cells(), active_before, "Brain should preserve knowledge");
    }
}
